...
const FILTER = {
  endPath: 'vlans', // will change to syslog later
  sinceTime: '',    // time in 12 digit long
  utilTime: '',     // time in 12 digit long
  prio: '7'
};
...
class SyslogPage extends Component {
  ...
  constructor(props) {
    ...
    this.SyslogEnum = {
      ...
      properties: {
        ...
        1: {name: 'alert', value: 0, text: t('alert')},
        2: {name: 'critical', value: 0, text: t('critical')},
        3: {name: 'error', value: 0, text: t('error')},
        4: {name: 'warning', value: 0, text: t('warning')},
        5: {name: 'notice', value: 0, text: t('notice')},
        6: {name: 'info', value: 0, text: t('info')},
        7: {name: 'debug', value: 0, text: t('debug')}
      }
    };
    this.SeverityEnum = {
      CRITICAL: 0,
      ...
      properties: {
        ...
        1: {name: 'warning', value: 0, text: t('warning')},
        2: {name: 'all', value: 0, text: t('all')}
      }
    };
  ...
  // Return Syslog prio as the highest in the severity range
  _getSyslogPrio = () => {
    let severity = this.SyslogEnum.ERROR;
    switch (this.sev) {
      case this.SeverityEnum.WARNING:
        severity = this.SyslogEnum.NOTICE;
        break;
      case this.SeverityEnum.ALL:
        severity = this.SyslogEnum.DEBUG;
        break;
      case this.SeverityEnum.CRITICAL:
      default:
        severity = this.SyslogEnum.ERROR;
        break;
    }
    return severity;
  };
  ...
  _getSinceTime = (fromDate, timeAgo) => {
  ...
  _buildFilter = () => {
    const url = FILTER;
    url.prio = this._getSyslogPrio();
    url.utilTime = new Date().getTime();
    url.sinceTime = this._getSinceTime(url.utilTime, this.time);
    return url;
  };
      ...
      <Box className="flex1">
        <Box className="pageBox flex0auto">
          <div float="right" position="relative">
              <Menu label={this.props.syslog.timeLabel} responsive="true"
                  selectable="true" inline="false" >
                <Anchor href="/#/syslog"
            ...
            <Button primary label={t('all')}
                onClick={this._onSeverityClick.bind(this,
                  this.SeverityEnum.ALL)} />
          </div>
        </Box>

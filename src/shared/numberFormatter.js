...
export function bpsToString(bitsPerSecond) {
  ...
  if (bitsPerSecond>=1000000000) {
    ...
    stringToBeReturned=`${(bitsPerSecond/1000000000)} Gbps`;
  } else if (bitsPerSecond>=1000000) {
    //Formula : Megabytes per second = bits per second ÷ 1,000,000
    stringToBeReturned=`${(bitsPerSecond/1000000)}  Mbps`;
  } else if (bitsPerSecond>=1000) {
    //Formula: Kilobytes per second = bits per second ÷ 1,000
    stringToBeReturned= `${bitsPerSecond} Kbps`;
  } else if (bitsPerSecond>0) {
    // 0 - 999 bps
    stringToBeReturned=`${bitsPerSecond} bps`;
  } else {
    // if 0 --> 0 bps
    stringToBeReturned=`${bitsPerSecond} bps`;
  }
